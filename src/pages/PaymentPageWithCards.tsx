...
import { pagosService, tarjetasService, reservasService } from '../services/api';
...
const PaymentPageWithCards: React.FC = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      console.log('Confirmando reserva ID:', paymentData.idReserva);
      try {
        await reservasService.confirmarReserva(paymentData.idReserva);
        console.log('Reserva confirmada exitosamente');
      } catch (updateError: any) {
        console.error('Error al confirmar reserva:', updateError);
        console.error('Detalle del error:', updateError.response?.data);
        throw updateError;
      }

      alert('Pago procesado exitosamente y reserva confirmada');
      navigate('/perfil');
