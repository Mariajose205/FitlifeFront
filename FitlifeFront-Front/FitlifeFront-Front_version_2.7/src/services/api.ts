...
// Base URLs para los microservicios - Conexión a través de Gateway
const BASE_URLS = {
  pagos: 'http://localhost:8090/api/pagos',
  locations: 'http://localhost:8090/api/locations',
  reservas: 'http://localhost:8090/api/reservas',
  notificaciones: 'http://localhost:8090/api/notificaciones',
  usuarios: 'http://localhost:8090/api/usuarios', // Autenticación a través del gateway
  auth: 'http://localhost:8090/auth' // Endpoint de autenticación del gateway
};
...
// Servicios de Usuarios (centralizado a través de Gateway)
export const usuariosService = {
  login: (credentials: { email: string; password: string }) => 
    axios.post(`${BASE_URLS.auth}/login`, credentials),
  register: (userData: any) => usuariosApi.post<UsuarioResponse>('/register', userData), // Directo a MS-usuarios para registro
  ...
  validateToken: (token: string) => axios.get(`${BASE_URLS.auth}/validate`, {
