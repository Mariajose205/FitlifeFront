...
import { setAuthenticatedUser } from '../utils/userTypeDetection';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await usuariosService.obtenerUsuarioPorEmail(email);
      } catch (userError: any) {
        // Si el usuario no existe, mostrar error y redirigir a registro
      ...
      // Si el usuario existe, intentar hacer login
      const response = await usuariosService.login({ email, password });
      const { token, user } = response.data;

      // Mapear usuario al formato esperado (el backend ya envía todos los campos)
      const mappedUser = {
        id: user.id.toString(),
        email: user.email,
        name: user.nombre,
      ...
      localStorage.setItem('welcome_message', `¡Bienvenido ${mappedUser.name}!`);

      // Redirigir a página de gimnasios para todos los usuarios
      navigate('/gimnasios');
      
    } catch (error: any) {
      console.error('Error de autenticación:', error);
      
      // Si es error de red, mantener mensaje anterior pero sin permitir login
      if (error.code === 'ERR_NETWORK' || error.code === 'ECONNREFUSED') {
        setError('No se puede conectar al servidor. Por favor, inténtalo más tarde.');
