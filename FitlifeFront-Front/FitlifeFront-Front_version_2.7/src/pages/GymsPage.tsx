...
import { MapPin, Phone, Clock, Star, Navigation, Search, Filter, Users, Dumbbell, Check, AlertCircle, X, Map, User } from 'lucide-react';
...
export const GymsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!isUserAuthenticated()) {
      ...
      localStorage.setItem('fitlife_redirect_after_login', '/gimnasios');
      navigate('/login');
      return;
    }
  }, [navigate]);
          ...
          {currentUser && (
            <div className="flex items-center space-x-2 mb-4">
              ...
              <span className="text-lg text-secondary-700">
                ¡Bienvenido, <span className="font-semibold text-secondary-900">{currentUser.name}</span>!
              </span>
            </div>
          )}
